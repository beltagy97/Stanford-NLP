...
class ParserModel(nn.Module):
    ...
    def embedding_lookup(self, w):
        ...
        for i,vector in enumerate(w):
          x[i]=(torch.index_select(self.embeddings, 0, vector.cuda(), out=None).reshape(-1))
        ### END YOUR CODE
        return x
    ...
    def forward(self, w):
        ...
        out = self.embedding_lookup(w).cuda()
        hidden = (torch.matmul(out,self.embed_to_hidden_weight) + self.embed_to_hidden_bias).cuda()
        hidden = F.relu(hidden)
        hidden = self.dropout(hidden)
        logits = (torch.matmul(hidden,self.hidden_to_logits_weight) + self.hidden_to_logits_bias).cuda()
