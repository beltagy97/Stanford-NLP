...
def minibatch_parse(sentences, model, batch_size):
    ...
    while unfinished_parses:
        ...
        for data,pred in zip(mini_batch,prediction):
            data.parse_step(pred)
            
        

        # update unfinished_parses
        unfinished_parses = []
        for item in partial_parses:
            if not(len(item.stack) == 1 and len(item.buffer) == 0):
                unfinished_parses.append(item)
